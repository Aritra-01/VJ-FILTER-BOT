...
async def start(client, message):
    ...
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        buttons = [[
            ...
            InlineKeyboardButton('✪ Uᴘᴅᴀᴛᴇs Cʜᴀɴɴᴇʟ', url="https://t.me/STK_UPDATES"),
            InlineKeyboardButton('⌬ Mᴏᴠɪᴇ Gʀᴏᴜᴘ', url='https://t.me/+CTC_jy4A5KE1Zjhl')
        ],[
    ...
    if len(message.command) != 2:
        if PREMIUM_AND_REFERAL_MODE == True:
            buttons = buttons = [[
                ...
                InlineKeyboardButton('😇 Bot Owner 😇', url="https://t.me/Soutick_09")
            ]]
        else:
            buttons = buttons = [[
                InlineKeyboardButton('⤬ Aᴅᴅ Mᴇ Tᴏ Yᴏᴜʀ Gʀᴏᴜᴘ ⤬', url=f'http://t.me/{temp.U_NAME}?startgroup=true')
    ...
    if len(message.command) == 2 and message.command[1] in ["subscribe", "error", "okay", "help"]:
        if PREMIUM_AND_REFERAL_MODE == True:
            buttons = [[
                ...
                InlineKeyboardButton('⌬ Aɴɪᴍᴇ Gʀᴏᴜᴘ', url=GRP_LNK)
            ],[
                InlineKeyboardButton('〄 Hᴇʟᴘ', callback_data='help'),
                InlineKeyboardButton('⍟ Aʙᴏᴜᴛ', callback_data='about')
            ],[
    ...
    if data.split("-", 1)[0] == "STK":
        user_id = int(data.split("-", 1)[1])
        STK = await referal_add_user(user_id, message.from_user.id)
        if STK and PREMIUM_AND_REFERAL_MODE == True:
    ...
    if data.split("-", 1)[0] == "BATCH":
        ...
        for msg in msgs:
            ...
            try:
                ...
                if STREAM_MODE == True:
                    button = [[
                        InlineKeyboardButton('❕Disclaimer❕', url="https://graph.org/Disclaimer-10-30-18")],
                        [
                        InlineKeyboardButton('Sᴜᴘᴘᴏʀᴛ Gʀᴏᴜᴘ', url="https://t.me/+CTC_jy4A5KE1Zjhl"),
                        InlineKeyboardButton('Uᴘᴅᴀᴛᴇs Cʜᴀɴɴᴇʟ', url=CHNL_LNK)
                    ],[
                        InlineKeyboardButton("𝗕𝗢𝗧 𝗢𝗪𝗡𝗘𝗥", url="t.me/Soutick_09")
                    ],[
                        InlineKeyboardButton('🚀 Fast Download / Watch Online🖥️', callback_data=f'generate_stream_link:{file_id}') #Don't change anything without contacting me @Soutick_09
                    ],[
                ...
                filesarr.append(msg)
                k = await client.send_message(chat_id = message.from_user.id, text=f"<b><u>Dude 😎 Read Below Note 👇</u></b>\n\nThis Anime File/Video will be deleted in <b><u>10 Mins</u> 🚀 <i></b>(Due to Copyright Issues)</i>.\n\n<b><i>Please forward this File/Video to your Saved Messages and Start Download there</i></b>")
        ...
        if is_valid == True:
            await message.reply_text(
                text=f"<b>Hey {message.from_user.mention}, You are successfully verified !\nNow you have unlimited access for all ANIMES till today midnight.</b>",
                protect_content=True
            )
...
<b>Your Ads token is expired, refresh your token and try again.</b>

<code>Token Validity Till 12 A.M. (IST)</code>

<b>What is token?</b>

<i>This is an ads token. If you pass 1 ad, you can use the bot unlimited times till 12 A.M. IST (Indian Standard Time) after passing the ad.</i>""",
                        protect_content=True,
                        reply_markup=InlineKeyboardMarkup(btn)
                    )
                    return
...
<i>This is an ads token. If you pass 1 ad, you can use the bot unlimited times till 12 A.M. IST (Indian Standard Time) after passing the ad.</i>""",
            ...
            title = '@STK_UPDATES  ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), file.file_name.split()))
            ...
            k = await msg.reply("<b><u>Dude 😎 Read Below Note 👇</u></b>\n\nThis Anime File/Video will be deleted in <b><u>10 Mins</u> 🚀 <i></b>(Due to Copyright Issues)</i>.\n\n<b><i>Please forward this File/Video to your Saved Messages and Start Download there</i></b>",quote=True)
            await asyncio.sleep(600)
            await msg.delete()
    ...
    files = files_[0]
    title = '[@STK_UPDATES]  ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))
    ...
    if CUSTOM_FILE_CAPTION:
        try:
            ...
            f_caption=f_caption
    if f_caption is None:
        f_caption = f"[@STK_UPDATES]  {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))}"
    if not await db.has_premium_access(message.from_user.id):
        if not await check_verification(client, message.from_user.id) and VERIFY == True:
            btn = [[
    InlineKeyboardButton(
...
async def shortlink(bot, message):
    ...
    await reply.edit_text(f"""<b>Successfully added shortlink API for {title}.\n\nCurrent Shortlink Website: <code>{shortlink_url}</code>\nCurrent API: <code>{api}</code>\n\nMade With ♥️ By <a href=https://t.me/Soutick_09>Soutick</a></b>""")
